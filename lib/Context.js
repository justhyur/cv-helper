...
export const ContextProvider = ({children}) => {
    ...
    const [latestRateUpdate, setLatestRateUpdate] = useState(0);
    const [upadtingRates, setUpdatingRates] = useState(false);
    useEffect(()=>{
        if( !upadtingRates && (Date.now() - latestRateUpdate) / 1000 / 60 > 10){
    ...
    const convert = (value, from, to, formatted) => {
        const converted = rates[from+to] ? rates[from+to]*value : value;
