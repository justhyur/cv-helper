...
export const ContextProvider = ({children}) => {
    ...
    const acceptBookings = (userName, password, numBookings) => {
        setIsBooking(true);
        const params = {
            token: SERVER_TOKEN,
            userName,
        ...
        const s = parseInt(numBookings) > 1 ? 's' : '';
        const popup = toast.loading(`Accepting booking${s}...`, toastOptions);
        const acceptPromise = axios.get(`${API_URL}/cv-prime/accept-invitations`, {params})
        .then((res) => {
            toast.dismiss(popup);
            if(res.data.meetings){
                setMeetings(res.data.meetings);
        ...
        .catch(err => {
            ...
            toast.update(popup, { 
                ...toastOptions, 
                render: `Error accepting meeting${s}: ${err.response?.data ?? err.message}`, 
                type: "error", 
                isLoading: false,
            });
            setIsBooking(false);
    ...
    return (
        <Context.Provider value={{
            ...
            preferredCurrency, convert, format, isBooking, bookMeetings, acceptBookings,
